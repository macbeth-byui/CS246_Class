...
public class EarthquakeAnalyzer {
    ...
    public void displayEarthquakeData() {
        ...
        Gson gson = new Gson();
        EarthquakeList earthquakes = gson.fromJson(data, EarthquakeList.class);
        Collections.sort(earthquakes.getEarthquakes(), new Comparator<EarthquakeEvent>() {
            public int compare(EarthquakeEvent o1, EarthquakeEvent o2) {
                return Float.compare(o1.getDetail().getMag(), o2.getDetail().getMag());
            }
        });
        for (EarthquakeEvent event : earthquakes.getEarthquakes()) {
